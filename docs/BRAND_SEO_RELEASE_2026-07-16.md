...
4. Store JSON-LD: added hasMap (Yandex Maps point for the shop) and priceRange to the LocalBusiness/Store block. sameAs stays empty until the owner confirms public profiles.
...
- `sameAs`: no identity URL is published until the owner confirms the exact Yandex Maps, 2GIS and social profiles. Search-result URLs are not valid identity links.
...
- Prometheus Blackbox already probes `/`, `/api/health`, `/catalog` and `/sitemap.xml`. The available Grafana currently has no alert rules and no working Telegram receiver; a delivery test needs an approved contact point/bot secret.
...
- Image: plumbing_store_v2-v2:brand-seo-20260716-v2 (built manually on the USA host from this exact tree; the pre-existing incomplete v1 tag is not reused).
- Compose: deploy/docker-compose.bluegreen-brand-seo-20260716.yml, service port 127.0.0.1:3016.
- Nginx upstream plumbing_store_v2_app: primary 3016, backup 3015 (previous seo-mobile release).
- Pre-cutover nginx backup: /etc/nginx/backups/plumbing_store.conf.backup-20260716-brand-seo.
...
Restore the nginx backup above, nginx -t, reload, verify public health against 3015. Do not remove the brand-seo container until rollback checks pass.
